-module(socks5_ipv6_to_ipv6_SUITE).
...
init_per_testcase(proxy_test, Config) ->
  ...
  Socket = socks5_ipv6_helpers:make_authorized_socket(Username, Password),
...
init_per_testcase(upd_associate_test, Config) ->
  ...
  Password = <<"testpassword">>,
  Socket = socks5_ipv6_helpers:make_authorized_socket(Username, Password),
  {ok, SendSocket} = gen_udp:open(0, [{active, false}, binary, inet6]),
